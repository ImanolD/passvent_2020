...
public lista: any = [];
  ...
  constructor(
    ...
    startChat(user){
      ...
      let sala =  {'index': this.generateUUID(), 'createdAt': new Date(), 'lastMsg': '¡Bienvenidos al chat de Passvent!', 'lastMsg_index': 'admin', 'members': [{'index': firebase.auth().currentUser.uid}, {'index': user.index}], 'messages': [{'message': '¡Bienvenidos al chat de Passvent!', 'senderId': 'admin'}]};
      ...
      this.af.list('Users/'+user.index+'/Chats').update(sala.index, {'index': sala.index})
          .then(()=>{
            ...
            this.alertCtrl.create({
              title: '¡Chat creado!',
              message: 'El chat ha sido creado, manda el primer mensaje 😉',
              buttons: ['Ok']
            });
          })
    }
